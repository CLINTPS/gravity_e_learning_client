...
  <div 
    className="absolute top-full left-0 mt-0 w-80 bg-white shadow-xl rounded-b-lg border-t-2 border-indigo-600 py-3"
  >
    <div className="px-5 pb-2">
      <h3 className="text-xs font-bold tracking-wider text-blue-700 uppercase mb-2">COURSE CATEGORY</h3>
    </div>
    
...
        >
          {category}
          <svg className="w-4 h-4 text-gray-400 group-hover:text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg>
        </Link>
      ))}
    </div>
  </div>
...
  <div 
    className="absolute top-full left-0 mt-0 w-64 bg-white shadow-xl rounded-b-lg border-t-2 border-indigo-600 py-3"
  >
    <div className="px-5 pb-2">
      <h3 className="text-xs font-bold tracking-wider text-blue-700 uppercase mb-2">TRAINING</h3>
    </div>
    
    <div className="max-h-96 overflow-y-auto relative">
      {categories.map((category) => (
        <Link
          key={category}
          to={`/training?target=${encodeURIComponent(category.replace(/ /g, ''))}`} 
          className="flex justify-between items-center px-5 py-3 text-gray-700 text-base hover:bg-gray-50 hover:text-indigo-600 transition duration-150 relative"
        >
          {category}
        </Link>
...
  >
    <div className="px-5 pb-2">
      <h3 className="text-xs font-bold tracking-wider text-blue-700 uppercase mb-2">RESOURCES</h3>
    </div>
    
    <div className="max-h-96 overflow-y-auto relative">
      {categories.map((category) => (
        <Link
          key={category}
          to={`/resources/${encodeURIComponent(category.replace(/ /g, '-').toLowerCase())}`} 
          className="flex justify-between items-center px-5 py-3 text-gray-700 text-base hover:bg-gray-50 hover:text-indigo-600 transition duration-150 relative"
        >
          {category}
        </Link>
...
  >
    <div className="px-5 pb-2">
      <h3 className="text-xs font-bold tracking-wider text-blue-700 uppercase mb-2">THE ACADEMY</h3>
    </div>
    
    <div className="max-h-96 overflow-y-auto relative">
      {categories.map((category) => (
        <Link
          key={category}
          to={`/academy/${encodeURIComponent(category.replace(/ /g, '-').toLowerCase())}`} 
          className="flex justify-between items-center px-5 py-3 text-gray-700 text-base hover:bg-gray-50 hover:text-indigo-600 transition duration-150 relative"
        >
          {category}
        </Link>
...
        <div 
            className="flex items-center h-16 gap-8 text-lg font-normal ml-12" 
...
          
          ...
          >
            ...
            {activeDropdown === 'courses' && <CoursesDropdown categories={COURSE_CATEGORIES} />}
          </div>
          ...
          >
            ...
            {activeDropdown === 'training' && <TrainingDropdown categories={TRAINING_CATEGORIES} />}
          </div>
          ...
          >
            ...
            {activeDropdown === 'resources' && <ResourcesDropdown categories={RESOURCE_CATEGORIES} />}
          </div>
          ...
          >
            ...
            {activeDropdown === 'academy' && <AcademyDropdown categories={ACADEMY_CATEGORIES} />}
          </div>
